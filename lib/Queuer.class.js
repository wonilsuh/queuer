...
function runTask() {
	...
	var length = this.useLocalStorage === true ? this.barn.llen(BARN_KEY) : this.queue.length;
	if (length > 0) {
...
var Queuer = function () {
  ...
  */
	...
	function Queuer() {
		...
		if (useLocalStorage === true) this.barn = new _barn2.default();
	}
  ...
  */
	...
	_createClass(Queuer, [{
		...
		value: function push(func) {
			...
			if (this.useLocalStorage === true) this.barn.rpush(BARN_KEY, toBeAdded);else queue.push(toBeAdded);
   ...
   */
		...
		value: function unshift(func) {
			...
			if (this.useLocalStorage === true) this.barn.lpush(BARN_KEY, toBeAdded);else this.queue.unshift(toBeAdded);
